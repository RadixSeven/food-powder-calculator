...
NUTRITION_MULTI_SYSTEM_PROMPT = (
    ...
    "shows a slice of the table. Some images may be CROPS focused "
    "tightly on the supplement-facts panel; others may be UNCROPPED "
    "source photos that show the whole product (hand, shelf, "
    "background) with the panel somewhere inside. Treat them all as "
    "views of the same product: ignore non-panel content (hands, "
    "shelves, other products in the background) and read the "
    "supplement-facts text wherever it appears. Combine the rows you "
    "can see across all photos into a single table, deduplicating "
    "rows that appear in more than one photo (matching by nutrient "
    "name when available, otherwise by row position within each "
    "photo). "
