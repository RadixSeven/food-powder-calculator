...
#
# Reformatting alone (`ruff format`) is never a failure: if it touches files,
# the user has nothing to do — the tree is now formatted. A failure means a
# gate produced something the user must act on (un-auto-fixable lint, type
# errors, test failures, coverage shortfall).
